...
export function VoyageProvider({ children }: { children: ReactNode }) {
  ...
  const outputs = useMemo(() => ({ currentFreight: result.freight.current, forecastFreight: result.freight.forecast, freightChange: result.freight.change, marketRisk: result.risk.score, selectedVessel: result.selected.name.toUpperCase(), portFit: result.port.score, portDelay: delayFor(state.portCongestion), estimatedSavings: result.strategy.expectedSavings, confidence: result.freight.confidence }), [result, state.portCongestion])
