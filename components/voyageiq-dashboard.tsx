...
function ForecastChart() {
  ...
  return (
    <div className="relative h-64 overflow-hidden rounded-md border border-[#d8dee8] bg-[#fbfcfe] p-4">
      ...
      <div className="absolute right-4 top-3 flex gap-1 rounded-md border border-[#d8dee8] bg-white p-1">{['7D', '15D', '30D'].map((item) => <button key={item} onClick={() => setRange(item)} className={`rounded px-2 py-1 text-[10px] font-semibold ${range === item ? 'bg-[#0b1f3a] text-white' : 'text-[#6b7b8f]'}`}>{item}</button>)}</div>
...
export default function VoyageIQDashboard() {
  ...
  return <div className="min-h-screen bg-[#f8fafd] text-[#0b1f3a]">
    ...
    <div className="lg:pl-[286px]"><header className="sticky top-0 z-20 border-b border-[#d8dee8] bg-white/95 backdrop-blur"><div className="flex h-[66px] items-center gap-3 px-4 md:px-7"><button onClick={() => setMobileOpen(true)} className="lg:hidden"><Menu className="size-5 text-[#0b1f3a]" /></button><div className="hidden items-center gap-2 text-xs text-[#718095] md:flex"><span className="font-semibold text-[#0b1f3a]">VoyageIQ</span><ChevronRight className="size-3" /> <span>{current.label}</span><ChevronRight className="size-3" /><span className="font-mono text-[#0b8ca2]">IDN → PAR</span></div><div className="ml-auto flex items-center gap-2"><div className="hidden items-center gap-2 rounded-md border border-[#e0e7ef] bg-[#fbfcfe] px-3 py-2 text-xs text-[#9aa6b5] xl:flex"><Search className="size-4" /> Search fixtures, vessels, ports, IMO...</div><button className="rounded-md p-2 text-[#708096] hover:bg-[#f1f5f9]"><Bell className="size-4" /></button><button className="hidden rounded-md p-2 text-[#708096] hover:bg-[#f1f5f9] sm:block"><CircleHelp className="size-4" /></button><div className="hidden h-7 w-px bg-[#e3e8ef] sm:block" /><button onClick={() => showToast('Charter execution queued for review')} className="hidden items-center gap-2 rounded-md bg-[#0b1f3a] px-3 py-2 text-[11px] font-semibold text-white sm:flex"><Zap className="size-3.5 text-[#58c7b3]" /> Execute Fixture</button></div></div><div className="flex gap-6 overflow-x-auto border-t border-[#edf0f4] px-4 py-2.5 text-[10px] font-medium text-[#708096] md:px-7"><span className="whitespace-nowrap font-semibold text-[#0b1f3a]">MARKET WATCH</span><span className="whitespace-nowrap">BDI <b className="font-mono text-[#0b1f3a]">1,842</b> <i className="not-italic text-[#0c9b86]">+1.8%</i></span><span className="whitespace-nowrap">IDN → PAR <b className="font-mono text-[#0b1f3a]">$21.40/MT</b> <i className="not-italic text-[#0c9b86]">+2.4%</i></span><span className="whitespace-nowrap">BRENT <b className="font-mono text-[#0b1f3a]">$82.40/bbl</b> <i className="not-italic text-[#db8a00]">−0.4%</i></span><span className="whitespace-nowrap">AIS FEED <b className="text-[#0c9b86]">● HEALTHY</b></span></div></header><main className="mx-auto max-w-[1500px] px-4 py-6 md:px-7 lg:px-9">{active === '/overview' ? <Overview onToast={showToast} /> : <ModulePage title={current.label} eyebrow={`Stage ${String(navItems.findIndex((item) => item.href === active) + 1).padStart(2, '0')} / Decision pipeline`} description={moduleDescription} onToast={showToast} />}</main></div>
    <ScenarioInputDrawer open={inputsOpen} onClose={() => setInputsOpen(false)} onApplied={() => onToast('Scenario updated')} />
    {toast && <div role="status" className="fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-md bg-[#0b1f3a] px-4 py-3 text-xs font-semibold text-white shadow-xl"><Check className="size-4 text-[#58c7b3]" /> {toast}</div>}
  </div>
}
