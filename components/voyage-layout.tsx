        ...
        <header className="sticky top-0 z-20 border-b border-[#d8dee8] bg-white/95 backdrop-blur">
          <div className="flex h-[66px] items-center gap-3 px-4 md:px-7">
            ...
            <div className="hidden items-center gap-2 text-xs text-[#718095] md:flex">
              ...
              <ChevronRight className="size-3" />
              <span className="font-mono text-[#0b8ca2]">{state.origin.split(',')[0].slice(0, 3).toUpperCase()} → {state.destination.split(' ')[0].slice(0, 3).toUpperCase()}</span>
            </div>
            ...
            <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[430px] flex-col border-l border-[#d8dee8] bg-white shadow-2xl">
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5">
                <div className="grid gap-4">
                  <label className="flex flex-col gap-1.5 text-[10px] font-semibold uppercase tracking-[.1em] text-[#718095]">Origin Port<select value={draft.origin} onChange={(e) => setDraft({ ...draft, origin: e.target.value })} className="rounded-md border border-[#d7e0ea] bg-[#fbfcfe] px-3 py-2.5 text-sm font-normal normal-case tracking-normal text-[#0b1f3a] outline-none focus:border-[#087cb8]"><option>Muara Berau, Indonesia</option><option>Taboneo, Indonesia</option><option>Samarinda, Indonesia</option><option>Newcastle, Australia</option><option>Richards Bay, South Africa</option></select></label>
                  <label className="flex flex-col gap-1.5 text-[10px] font-semibold uppercase tracking-[.1em] text-[#718095]">Destination Port<select value={draft.destination} onChange={(e) => setDraft({ ...draft, destination: e.target.value })} className="rounded-md border border-[#d7e0ea] bg-[#fbfcfe] px-3 py-2.5 text-sm font-normal normal-case tracking-normal text-[#0b1f3a] outline-none focus:border-[#087cb8]"><option>Paradip Port, India</option><option>Mundra Port, India</option><option>Krishnapatnam Port, India</option><option>Kandla Port, India</option><option>Visakhapatnam Port, India</option></select></label>
                  <div className="grid grid-cols-2 gap-3"><label className="flex flex-col gap-1.5 text-[10px] font-semibold uppercase tracking-[.1em] text-[#718095]">Commodity<select value={draft.commodity} onChange={(e) => setDraft({ ...draft, commodity: e.target.value })} className="rounded-md border border-[#d7e0ea] bg-[#fbfcfe] px-3 py-2.5 text-sm font-normal normal-case tracking-normal text-[#0b1f3a] outline-none focus:border-[#087cb8]"><option>Thermal Coal</option><option>Coking Coal</option><option>Iron Ore</option><option>Bauxite</option><option>Grain</option></select></label><label className="flex flex-col gap-1.5 text-[10px] font-semibold uppercase tracking-[.1em] text-[#718095]">Cargo MT<input type="number" min="1" value={draft.cargoQuantity} onChange={(e) => setDraft({ ...draft, cargoQuantity: Math.max(1, Number(e.target.value) || 1) })} className="rounded-md border border-[#d7e0ea] bg-[#fbfcfe] px-3 py-2.5 font-mono text-sm font-normal normal-case tracking-normal text-[#0b1f3a] outline-none focus:border-[#087cb8]" /></label></div>
