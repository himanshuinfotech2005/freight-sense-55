...
export default function VoyagePlannerPage() {
  const { state, updateState, liveOutputs } = useVoyage()
  ...
  const updateLaycan = (next: { start: string; end: string }) => {
    setLaycanDraft(next)
    if (!next.start || !next.end || next.start > next.end) return
    updateState({ laycanStart: next.start, laycanEnd: next.end, laycanWindow: formatLaycan(next.start, next.end) })
  }

  const applyLaycan = () => {
    if (laycanDraft.start > laycanDraft.end) return
    updateLaycan(laycanDraft)
    setLaycanOpen(false)
  }
